...
CExtendAudioFrameObserver::CExtendAudioFrameObserver():agoraAudioBuf(new AudioCircularBuffer<char>(2048,true)), play_back_audio_circular_buffer_(new AudioCircularBuffer<char>(2048, true))
{
...
void CExtendAudioFrameObserver::setRemoteVolume(int volume) {
	mtx.lock();
	if (volume < 0)
	{
		mtx.unlock();
...
void CExtendAudioFrameObserver::setPlayoutSignalVolume(int volume)
{
	mtx.lock();
	if (volume < 0)
	{
